# 14. Host Agent Package And Dependency Policy

## Base Package

- `com.example.springai.host`

## Dependency Rules

- `controller -> service -> orchestrator -> ports(plan/invoke/compose/store/graph)`
- `invoke -> a2a client/registry` only
- `plan/compose -> runtime(DefaultAgentLlmRuntime)` only
...
- 실패 응답은 host 표준 메시지로 정규화한다.
- 호스트 진입점은 `/a2a/host/*`만 사용한다.
